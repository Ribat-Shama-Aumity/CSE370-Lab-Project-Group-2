...
// A room tour can only be requested by a student who has booked the room.
...
if (!$booking_result || mysqli_num_rows($booking_result) == 0) {
    header("Location: room.php?id=" . $safe_id . "&tour_error=bookfirst");
    exit();
}
...
if ($stmt) {
    mysqli_stmt_bind_param($stmt, "iiiss", $listing_id, $std_id, $provider_id, $tour_date, $tour_time);
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        if ($notice_stmt) {
            mysqli_stmt_bind_param($notice_stmt, "iis", $provider_id, $tour_id, $message);
